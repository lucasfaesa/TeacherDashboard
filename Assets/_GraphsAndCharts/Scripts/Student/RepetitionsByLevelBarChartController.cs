...
{
    public class RepetitionsByLevelBarChartController : MonoBehaviour
    {
        ...
        private void FillBarChart(StudentAndCompleteLevelsSessionsDTO studentSession)
        {
            ...
            var completeLevelSessions = studentSession.CompleteLevelSessions.OrderBy(x => x.TotalRepetitions).ToList();

            if (!completeLevelSessions.Exists(x => x.TotalRepetitions > 0))
            {
            ...
            for (var i = 0; i < completeLevelSessions.Count; i++)
            {
                if (completeLevelSessions[i].LevelId != null)
                {
                    ...
                    barChart.DataSource.SetValue(levelNameAndNumber, "All", completeLevelSessions[i].TotalRepetitions);
                }
                if (completeLevelSessions[i].QuizId != null)
                {
                    ...
                    barChart.DataSource.SetValue(quizNameOnChart, "All", completeLevelSessions[i].TotalRepetitions);
                }
            }
