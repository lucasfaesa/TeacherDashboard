...
{
    /// <summary>
    ///     Base class for canvas series graphic object. Contains functionallity that enable creating a hovering prefab where
    ///     the mouse is located , and event handling
    ///     The main idea behind this implementation is that it allows to create a mesh once. Then in order to create the hover
    ///     effect , an object is placed on top of that mesh , those leaving the mesh unmodified.
    /// </summary>
    public abstract class EventHandlingGraphic : MaskableGraphic
    {
        ...
        /// </summary>
        private SensitivityControl mControl;

        /// <summary>
        ///     The currently hovered item
        /// </summary>
        private ChartItemEffect mCurrentHover;
        ...
        /// </summary>
        private readonly List<ChartItemEffect> mHoverFreeObjects = new();

        /// <summary>
        ///     All the objects that have a hover animation displayed. Only one object can be hovered. But the hover animation has
        ///     a fade effect so multiple objects can display the animation at the same time
        /// </summary>
        private readonly List<ChartItemEffect> mHoverObjectes = new();

        /// <summary>
        ///     The prefab for the hover effect
        /// </summary>
        private ChartItemEffect mHoverPrefab;

        private Transform mHoverTransform;
        private bool mIsMouseIn;
        private Vector2 mLastMousePosition;
        private object mPickedData;

        private int mPickedIndex = -1, mPickedType = -1;
        protected Rect? mUvRect;

        protected Rect? ViewRect;

        /// <summary>
        ///     When the graphic data is clliped , this representes the first index being displayed in the graph. Thus the [Data
        ///     Index] = refrenceIndex + [Graphic Index]
        /// </summary>
        ...
        /// <summary>
        ///     returns the threshold at which the mouse is considered in the area of the graphic. This threshold is realtive to
        ...
        /// <summary>
        ///     call this to free all hover objects and reset event input. This is useful mostly when the entire data of the
        ///     graphic has been replaced and old animations should be discarded
        /// </summary>
        public void ClearEvents()
        {
            mIsMouseIn = false;
            for (var i = 0; i < mHoverObjectes.Count; i++)
            {
        ...
        ///     This is useful for features such as realtime graph. Where new objects can be picked even if the mouse is in one
        ///     place only
        /// </summary>
        ...
        /// <summary>
        ///     This method is called during update if RefreshInputs has been called in the preivous frame. When the canvas graph
        ///     mesh updates , all active hover effects are repositioned to match their new position.
        /// </summary>
        ...
        /// </summary>
        /// <param name="hover"></param>
        private void TriggerOut(ChartItemEffect hover)
        {
            hover.TriggerOut(true);
            var control = hover.GetComponent<ChartMaterialController>();
            if (control)
                control.TriggerOff();
        }
        ...
        /// </summary>
        /// <param name="hover"></param>
        private void TriggerIn(ChartItemEffect hover)
        {
        ...
        {
            var count = mHoverFreeObjects.Count;
            ...
            else
            {
                if (mHoverPrefab == null)
                    return null;
                var obj = Instantiate(mHoverPrefab.gameObject);
                var g = obj.GetComponent<MaskableGraphic>();
                // if (g != null)
                //     g.maskable = false;
        ...
        /// </summary>
        /// <param name="hover"></param>
        private void SetUpHoverObject(ChartItemEffect hover)
        {
            if (hover == null)
                return;
            var pickedIndex = hover.ItemIndex - refrenceIndex;
            if (pickedIndex < 0)
                return;
        ...
        private void DoMouse(Vector3 mouse, bool leave, bool force)
        {
            var prevI = mPickedIndex;
            var prevJ = mPickedType;
            if (leave)
                DoLeave();
            else
        ...
        public bool HandleMouseMove(bool force)
        {
            ...
            var checkCamera = mCaster.eventCamera;
            if (checkCamera == null)
            ...
            var thresh = MouseInThreshold;
            if (force) SetUpAllHoverObjects();

            var min = Min;
            var max = Max;
            if (mousePos.x < min.x - thresh || mousePos.y < min.y - thresh || mousePos.x > max.x + thresh ||
                mousePos.y > max.y + thresh)
            {
                if (mIsMouseIn)
                {
                    mIsMouseIn = false;
            ...
            if (pointer != null && pointer.Click) HandleMouseDown();
            return true;
        }
        ...
        private void HandleMouseDown()
        {
            ...
            if (mPickedIndex != -1 && mPickedType != -1)
                if (Click != null)
                {
                    ...
                    RectTransformUtility.ScreenPointToLocalPointInRectangle(transform as RectTransform, checkMousePos,
                        mCaster.eventCamera, out mousePos);
                    var pos = transform.InverseTransformPoint(mousePos);
