        ...
        protected override GameObject CreateCategoryObject(Vector3[] path, int category)
        {
            ...
            if (mCategoryLabels != null && mCategoryLabels.isActiveAndEnabled)
            {
                for (int i = 0; i < path.Length - 1; i++)
                {
                    ...
                    string toSet = mCategoryLabels.TextFormat.Format(
                        ChartAdancedSettings.Instance.FormatFractionDigits(fractionDigits, val, CustomNumberFormat),
                        cat.Name, group);
                    BillboardText billboard = ChartCommon.CreateBillboardText(null, mCategoryLabels.TextPrefab,
                        transform, toSet, labelPos.x, labelPos.y, labelPos.z, 0f, null, hideHierarchy,
                        mCategoryLabels.FontSize, mCategoryLabels.FontSharpness);
