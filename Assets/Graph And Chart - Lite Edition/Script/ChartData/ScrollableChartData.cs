...
namespace ChartAndGraph
{
    ...
    {
        [SerializeField] private double automaticcHorizontaViewGap;

        /// <summary>
        ///     set this to true to automatically detect the horizontal size of the graph chart
        /// </summary>
        [SerializeField] private bool automaticHorizontalView = true;

        /// <summary>
        ///     set this to true to automatically detect the horizontal size of the graph chart
        /// </summary>
        [SerializeField] private bool automaticVerticallView = true;


        [SerializeField] private double automaticVerticalViewGap;


        [SerializeField] private double horizontalViewOrigin;

        [SerializeField] private double horizontalViewSize = 100;

        protected Dictionary<string, BaseScrollableCategoryData> mData = new();
        protected List<BaseSlider> mSliders = new();

        protected bool mSuspendEvents;

        [SerializeField] private double verticalViewOrigin;
        ...
        {
            get => automaticcHorizontaViewGap;
            set
            {
        ...
        public virtual double GetMaxValue(int axis, bool dataValue)
        {
            if (dataValue == false)
            {
                ...
                {
                    var axisAdd = horizontalViewSize;
            ...
            double? res = null;
            double add = 0;
            foreach (var cat in mData.Values)
            {
            ...
            for (var i = 0; i < mSliders.Count; i++)
            {
                var s = mSliders[i];
                if (axis == 0)
            ...
            if (res.HasValue == false)
                return 10f;
            var gap = axis == 0 ? automaticcHorizontaViewGap : automaticVerticalViewGap;
        ...
        public virtual double GetMinValue(int axis, bool dataValue)
        {
            ...
            if (res.HasValue == false)
                return 0.0;
            var max = GetMaxValue(axis, dataValue);
            if (max == res.Value)
            {
                if (res.Value == 0.0)
                    return -10.0f;
        ...
        public void RestoreDataValues(int axis)
        {
            if (axis == 0)
            {
                var maxH = GetMaxValue(0, true);
                var minH = GetMinValue(0, true);
                horizontalViewOrigin = minH;
                horizontalViewSize = maxH - minH;
            }
            else
            {
                var maxV = GetMaxValue(1, true);
                var minV = GetMinValue(1, true);
