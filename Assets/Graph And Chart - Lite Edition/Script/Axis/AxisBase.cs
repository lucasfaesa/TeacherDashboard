...
#pragma warning disable 0414
        ...
        /// </summary>
        [SerializeField] private bool SimpleView = true;
#pragma warning restore 0414
        /// <summary>
        ///     the format of the axis labels. This can be either a number, time or date time. If the selected value is either
        ///     DateTime or Time , user ChartDateUtillity to convert dates to double values that can be set to the chart.
        /// </summary>
        [SerializeField]
        [Tooltip(
            "the format of the axis labels. This can be either a number, time or date time. If the selected value is either DateTime or Time , user ChartDateUtillity to convert dates to double values that can be set to the graph")]
        private AxisFormat format;

        [SerializeField] [Tooltip("")] private bool withEdges2 = true;

        /// <summary>
        ///     the depth of the axis reltive to the chart position
        /// </summary>
        [SerializeField] [Tooltip("the depth of the axis reltive to the chart position")]
        private AutoFloat depth;

        [SerializeField] [Tooltip("The main divisions of the chart axis")] [FormerlySerializedAs("MainDivisions")]
        private ChartMainDivisionInfo mainDivisions = new();

        [SerializeField] [Tooltip("The sub divisions of each main division")] [FormerlySerializedAs("SubDivisions")]
        private ChartSubDivisionInfo subDivisions = new();


        private readonly Dictionary<double, string> mFormats = new();
        private readonly List<double> mTmpToRemove = new();
        ...
        /// </summary>
        public ChartMainDivisionInfo MainDivisions => mainDivisions;
        ...
        /// <summary>
        ///     checks that all properies of this instance have valid values. This method is used internally and should not be
        ///     called
        /// </summary>
        ...
        private void GetStartEnd(AnyChart parent, ChartOrientation orientation, float total, out float start,
            out float end)
        {
            start = 0;
            end = total;
        ...
        private void DrawCustomDivisions(double scrollOffset, AnyChart parent, Transform parentTransform,
            ChartDivisionInfo info, IChartMesh mesh, int group, ChartOrientation orientation, bool oppositeSide,
            double mainGap)
        {
            //scrollOffset = -scrollOffset;
            double parentSize = orientation == ChartOrientation.Vertical
            ...
            GetDirectionVectors(parent, info, orientation, 0f, oppositeSide, out startPosition, out lengthDirection,
                out advanceDirection);
            ...
            double backLength = orientation == ChartOrientation.Vertical
                ? ((IInternalUse)parent).InternalTotalWidth
                : ((IInternalUse)parent).InternalTotalHeight;
            ...
            var totaluv = Math.Abs(length);
            if (backLength != 0 && markDepth > 0)
                totaluv += Math.Abs(backLength) + Math.Abs(markDepth);

            var halfThickness = advanceDirection * (info.MarkThickness * 0.5f);
            // if (scrollOffset != 0f)
            //     last--;

            var hasValues = ((IInternalUse)parent).InternalHasValues(this);
            var maxValue = ((IInternalUse)parent).InternalMaxValue(this);
            var minValue = ((IInternalUse)parent).InternalMinValue(this);
            var range = maxValue - minValue;

            var AutoAxisDepth = Depth.Value;
            //float scrollFactor = (scrollOffset / (float)(maxValue - minValue));
            //scrollOffset = scrollFactor * parentSize;

            if (Depth.Automatic) AutoAxisDepth = (float)(((IInternalUse)parent).InternalTotalDepth - markDepth);

            var startValue = scrollOffset + minValue;
            var endValue = scrollOffset + maxValue + double.Epsilon;
            var direction = 1.0;
            Func<double, double> ValueToPosition = x => (x - startValue) / range * parentSize;
            if (startValue > endValue) direction = -1.0;
            //ValueToPosition = x => (1.0- ((x - startValue) / range)) * parentSize;
            mTmpToRemove.Clear();
            foreach (var key in mFormats.Keys)
                if (key * direction > endValue * direction || key * direction < startValue * direction)
                    mTmpToRemove.Add(key);
            for (var k = 0; k < mTmpToRemove.Count; k++)
                mFormats.Remove(mTmpToRemove[k]);
            var customValues = orientation == ChartOrientation.Vertical
                ? ((IInternalUse)parent).VerticalCustomAxis
                : ((IInternalUse)parent).HorizontalCustomAxis;
            foreach (var current in customValues)
            {
                var offset = ValueToPosition(current);
                if (offset < 0 || offset > parentSize)
                    continue;
                var start = startPosition + advanceDirection * offset;
                var size = halfThickness + length * lengthDirection;
                start -= halfThickness;
                //size += halfThickness;
                var uvoffset = 0f;

                var r = ChartCommon.FixRect(new Rect((float)start.x, (float)start.y, (float)size.x, (float)size.y));
                ...
                if (hasValues)
                {
                    var val = Math.Round(current * 1000.0) / 1000.0;
                    var toSet = "";
                    var keyVal = val; // (int)Math.Round(val);
                    var dic = orientation == ChartOrientation.Horizontal
                        ? parent.HorizontalValueToStringMap
                        : parent.VerticalValueToStringMap;
                        ...
                        if (mFormats.TryGetValue(val, out toSet) == false)
                        {
                            if (format == AxisFormat.Number)
                            {
                                toSet = ChartAdancedSettings.Instance.FormatFractionDigits(info.FractionDigits, val,
                                    parent.CustomNumberFormat);
                            }
                            else
                            {
                                var date = ChartDateUtility.ValueToDate(val);
                    ...
                    var textPos = new DoubleVector3(start.x, start.y);
                    textPos += lengthDirection * info.TextSeperation;
                    var userData = new TextData();
                    userData.interp = (float)(offset / parentSize);
                    userData.info = info;
                    userData.fractionDigits = info.FractionDigits;
                    mesh.AddText(parent, info.TextPrefab, parentTransform, info.FontSize, info.FontSharpness, toSet,
                        (float)textPos.x, (float)textPos.y, AutoAxisDepth + info.TextDepth, 0f, userData);
                }
                ...
                if (markDepth > 0)
                {
                    if (orientation == ChartOrientation.Horizontal)
                    {
                        ...
                        r = ChartCommon.FixRect(
                            new Rect((float)start.x, AutoAxisDepth, (float)size.x, (float)markDepth));
                    ...
                    else
                    {
                        SetMeshUv(mesh, (float)(-markDepth / totaluv), uvoffset);
                        r = ChartCommon.FixRect(
                            new Rect((float)start.y, AutoAxisDepth, (float)size.y, (float)markDepth));
                    ...
                    if (backLength != 0)
                    {
                        ...
                        uvoffset += Math.Abs(mesh.Length);
                        var backSize = halfThickness + backLength * lengthDirection;
                        var backR = ChartCommon.FixRect(new Rect((float)start.x, (float)start.y, (float)backSize.x,
                            (float)backSize.y));
        ...
        private void DrawDivisions(double scrollOffset, AnyChart parent, Transform parentTransform,
            ChartDivisionInfo info, IChartMesh mesh, int group, ChartOrientation orientation, double gap,
            bool oppositeSide, double mainGap)
        {
            //scrollOffset = -scrollOffset;
            double parentSize = orientation == ChartOrientation.Vertical
                ? ((IInternalUse)parent).InternalTotalHeight
...
//            float scrollFactor = (scrollOffset / (float)(maxValue - minValue));
            ...
            //ValueToPosition = x => (1.0- ((x - startValue) / range)) * parentSize;
            gap = Math.Abs(gap);
            var fraction = gap - (scrollOffset - Math.Floor(scrollOffset / gap - double.Epsilon) * gap);
            ...
            mTmpToRemove.Clear();
            var startRange = startValue + fraction;
            foreach (var key in mFormats.Keys)
            ...
            foreach (double current in
                     new DivisionEnumerable(startValue, fraction, direction, gap, endValue, WithEdges))
            {
                ++i;
                if (i > 3000) break;
                ...
                if (mainGap > 0.0)
                {
                    ...
                    if (current > currentMain) currentMain += mainGap;
                }
...
private void GetDirectionVectors(AnyChart parent, ChartDivisionInfo info, ChartOrientation orientation,
            float scrollOffset, bool oppositeSide, out DoubleVector3 startPosition, out DoubleVector3 lengthDirection,
            out DoubleVector3 advanceDirection)
        {
            if (orientation == ChartOrientation.Horizontal)
            {
        ...
        internal void AddCustomDivisionsToChartMesh(double scrollOffset, AnyChart parent, Transform parentTransform,
            IChartMesh mesh, ChartOrientation orientation)
        {
            var maxValue = ((IInternalUse)parent).InternalMaxValue(this);
            var minValue = ((IInternalUse)parent).InternalMinValue(this);
            var range = maxValue - minValue;
            ...
            if ((MainDivisions.Alignment & ChartDivisionAligment.Opposite) == ChartDivisionAligment.Opposite)
                DrawCustomDivisions(scrollOffset, parent, parentTransform, MainDivisions, mesh, 0, orientation, false,
                    -1);
            if ((MainDivisions.Alignment & ChartDivisionAligment.Standard) == ChartDivisionAligment.Standard)
                DrawCustomDivisions(scrollOffset, parent, parentTransform, MainDivisions, mesh, 0, orientation, true,
                    -1);
        }
        ...
        /// <param name="mesh"></param>
        /// <param name="orientation"></param>
        internal void AddSubdivisionToChartMesh(double scrollOffset, AnyChart parent, Transform parentTransform,
            IChartMesh mesh, ChartOrientation orientation)
        {
            var total = SubDivisions.Total;
            if (total <= 1) // no need for more
                return;
            var maxValue = ((IInternalUse)parent).InternalMaxValue(this);
            var minValue = ((IInternalUse)parent).InternalMinValue(this);
            var range = maxValue - minValue;
            var gap = GetMainGap(parent, range);
            if (gap.HasValue == false)
                return;
            var subGap = gap.Value / total;
            ...
            if ((SubDivisions.Alignment & ChartDivisionAligment.Opposite) == ChartDivisionAligment.Opposite)
                DrawDivisions(scrollOffset, parent, parentTransform, SubDivisions, mesh, 0, orientation, subGap, false,
                    gap.Value);
            if ((SubDivisions.Alignment & ChartDivisionAligment.Standard) == ChartDivisionAligment.Standard)
                DrawDivisions(scrollOffset, parent, parentTransform, SubDivisions, mesh, 0, orientation, subGap, true,
                    gap.Value);
        }

        private double? GetMainGap(AnyChart parent, double range)
        {
            double gap = MainDivisions.UnitsPerDivision;
            if (MainDivisions.Messure == ChartDivisionInfo.DivisionMessure.TotalDivisions)
            {
                var total = MainDivisions.Total;
                if (total <= 0)
                    return null;
                gap = range / total;
            }
        ...
        /// <param name="mesh"></param>
        /// <param name="orientation"></param>
        internal void AddMainDivisionToChartMesh(double scrollOffset, AnyChart parent, Transform parentTransform,
            IChartMesh mesh, ChartOrientation orientation)
        {
            var maxValue = ((IInternalUse)parent).InternalMaxValue(this);
            var minValue = ((IInternalUse)parent).InternalMinValue(this);
            ...
            mesh.Tile = ChartCommon.GetTiling(MainDivisions.MaterialTiling);
            if ((MainDivisions.Alignment & ChartDivisionAligment.Opposite) == ChartDivisionAligment.Opposite)
                DrawDivisions(scrollOffset, parent, parentTransform, MainDivisions, mesh, 0, orientation, gap.Value,
                    false, -1);
            if ((MainDivisions.Alignment & ChartDivisionAligment.Standard) == ChartDivisionAligment.Standard)
                DrawDivisions(scrollOffset, parent, parentTransform, MainDivisions, mesh, 0, orientation, gap.Value,
                    true, -1);
        ...
        private class DivisionEnumerable : IEnumerable
        {
            private readonly double mStartValue;
            private readonly double mFraction;
            private readonly double mDirection;
            private readonly double mGap;
            private readonly double mEndValue;
            private readonly bool mWithSides;

            public DivisionEnumerable(double startValue, double fraction, double direction, double gap, double endValue,
                bool withSides)
            {
            ...
            {
                var startRange = mStartValue + mFraction;
                if (Math.Abs(mFraction) < 0.001f && mWithSides)
                    yield return mStartValue;
                for (var current = startRange;
                     mDirection * current < mDirection * mEndValue;
                     current += mGap * mDirection)
