...
{
    public class TotalPlaysByLevelBarChartController : MonoBehaviour
    {
        ...
        private void FillBarChart(StudentAndCompleteLevelsSessionsDTO studentSession)
        {
            ...
            var completeLevelSessions = studentSession.CompleteLevelSessions.OrderBy(x => x.TotalPlays).ToList();

            if (!completeLevelSessions.Exists(x => x.TotalPlays > 0))
            {
            ...
            for (var i = 0; i < completeLevelSessions.Count; i++)
            {
                if (completeLevelSessions[i].LevelId != null)
                {
                    ...
                    barChart.DataSource.SetValue(levelNameAndNumber, "All", completeLevelSessions[i].TotalPlays);
                }
                ...
                if (completeLevelSessions[i].QuizId != null)
                {
                    ...
                    barChart.DataSource.SetValue(quizNameOnChart, "All", completeLevelSessions[i].TotalPlays);
                }
            }
