...
{
    public class MaxScoreByLevelBarChartController : MonoBehaviour
    {
        ...
        {
            if (studentSession.CompleteLevelSessions.Count == 0)
            {
            ...
            var completeLevelSessions = studentSession.CompleteLevelSessions.OrderBy(x => x.MaxScore).ToList();

            if (!completeLevelSessions.Exists(x => x.MaxScore > 0))
            {
                noDataText.SetActive(true);
                barChart.DataSource.EndBatch();
            ...
            for (var i = 0; i < completeLevelSessions.Count; i++)
            {
                if (completeLevelSessions[i].LevelId != null)
                {
                    ...
                    barChart.DataSource.SetValue(levelNameAndNumber, "All", completeLevelSessions[i].MaxScore);
                }
                if (completeLevelSessions[i].QuizId != null)
                {
                    ...
                    barChart.DataSource.SetValue(quizNameOnChart, "All", completeLevelSessions[i].MaxScore);
                }
            }
