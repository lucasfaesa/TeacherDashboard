...
namespace ChartAndGraph
{
    ...
    public class BarData : AbstractChartData, IInternalBarData, IChartData
    {
        ...
        /// <summary>
        ///     set this to true to automatically detect the highest bar value int the chart. This value will be used to scale all
        ///     other bars
        /// </summary>
        [SerializeField] private bool automaticMaxValue = true;

        /// <summary>
        ///     Mannualy set the maximum value , all bars will be scaled based on this value. If a bar value is larger than this
        ...
        /// <summary>
        ///     set this to true to automatically detect the lowest bar value int the chart. This value will be used to scale all
        ///     other bars
        /// </summary>
        [SerializeField] private bool automaticMinValue;

        /// <summary>
        ///     Mannualy set the minimum value , all bars will be scaled based on this value. If a bar value is larger than this
        ///     value it will be clamped
        ...
        ChartSparseDataSource IInternalBarData.InternalDataSource => mDataSource;
        ...
        public void OnAfterDeserialize()
        {
            ...
            for (var i = 0; i < mCategories.Length; i++) AddCategory(mCategories[i].Name, mCategories[i].Materials);
        ...
        {
            var catgories = mDataSource.Columns.Select(x => x.Name).ToArray();
            var groups = mDataSource.Rows.Select(x => x.Name).ToArray();
        ...
        {
            var catgories = mDataSource.Columns.Select(x => x.Name).ToArray();
            foreach (var s in catgories) RemoveCategory(s);
        }
        ...
        {
            var col = mDataSource.mColumns[name];
            var values = new double[TotalGroups];
            for (var i = 0; i < TotalGroups; i++)
            {
                var g = GetGroupName(i);
