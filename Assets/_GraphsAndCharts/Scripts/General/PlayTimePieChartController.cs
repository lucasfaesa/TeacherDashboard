...
{
    public class PlayTimePieChartController : MonoBehaviour
    {
        ...
        private void FillPieChart(List<CompleteStudentSessionDTO> dto)
        {
            ...
            dto = dto.OrderBy(x => x.TotalElapsedTime).ToList();

            if (!dto.Exists(x => x.TotalElapsedTime > 0))
            {
            ...
            if (!useDebug)
            {
                ...
                for (var i = 0; i < dto.Count; i++)
                {
                    ...
                    else if (i != 0)
                    {
                        ...
                        Color newColor = new Color32((byte)currentColorValue, 255, (byte)currentColorValue, 255);
                        newMat.SetColor(Color1, newColor);
                    }

                    pieChart.DataSource.AddCategory(dto[i].Student.Name, newMat);
                    pieChart.DataSource.SetValue(dto[i].Student.Name,
                        dto[i].TotalElapsedTime > 60 ? dto[i].TotalElapsedTime / 60 : 1);
                }*/
                ...
                for (var i = 0; i < dto.Count; i++)
                {
                    ...
                    pieChart.DataSource.AddCategory(dto[i].Student.Name, newMat);
                    pieChart.DataSource.SetValue(dto[i].Student.Name,
                        (dto[i].TotalElapsedTime / 60) == 0 ? 1 : dto[i].TotalElapsedTime / 60);
                }
            }
            else
            {
                ...
                for (var i = 0; i < dto.Count; i++)
                {
                    ...
                    pieChart.DataSource.SetValue(dto[i].Student.Name + cont,
                        (dto[i].TotalElapsedTime / 60) == 0 ? 1 : dto[i].TotalElapsedTime / 60);
