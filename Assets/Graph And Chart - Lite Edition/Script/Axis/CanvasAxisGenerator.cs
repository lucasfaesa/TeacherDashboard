...
namespace ChartAndGraph.Axis
{
    ...
    [ExecuteInEditMode]
    internal class CanvasAxisGenerator : Image, IAxisGenerator
    {
        private AxisBase mAxis;
        private Mesh mCleanMesh;
        private Material mDispose;
        private int mDivType;
        private MeshFilter mFilter;
        private Material mMaterial;
        private CanvasChartMesh mMesh;
        private ChartOrientation mOrientation;
        private AnyChart mParent;
        private MeshRenderer mRenderer;
        private double mScrollOffset;
        private List<BillboardText> mTexts;
        private float mTiling = 1f;
...
//#pragma warning restore 0672
        ...
        public void FixLabels(AnyChart parent)
        {
            if (mAxis == null)
                return;
            if (mAxis.MainDivisions.Messure == ChartDivisionInfo.DivisionMessure.DataUnits)
            {
                SetAxis(mScrollOffset, mParent, mAxis, mOrientation, mDivType);
                return;
            }

            for (var i = 0; i < mTexts.Count; i++)
            {
                var text = mTexts[i];
                var min = ((IInternalUse)parent).InternalMinValue(mAxis);
                var max = ((IInternalUse)parent).InternalMaxValue(mAxis);
                if (text.UserData is AxisBase.TextData)
                {
                    var data = (AxisBase.TextData)text.UserData;
                    var newVal = min * (1.0 - data.interp) + max * data.interp;
                    var toSet = "";
                    ...
                    {
                        var date = ChartDateUtility.ValueToDate(newVal);
            ...
            int divType)
        {
            ...
            if (mMesh.TextObjects != null)
                foreach (var text in mMesh.TextObjects)
        ...
        private float GetTiling(MaterialTiling tiling)
        {
            ...
            var length = ChartCommon.GetAutoLength(mParent, mOrientation, inf);
