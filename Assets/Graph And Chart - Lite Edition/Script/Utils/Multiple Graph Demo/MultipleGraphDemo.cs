...
public class MultipleGraphDemo : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        var animationPoints = new List<DoubleVector2>();
        var x = 0f;
        ...
        Graph.DataSource
            .ClearCategory(
                "Player 2"); // clear the "Player 2" category. this category is defined using the GraphChart inspector

        for (var i = 0; i < TotalPoints; i++) //add random points to the graph
        {
            Graph.DataSource.AddPointToCategory("Player 1", DateTime.Now + TimeSpan.FromDays(x),
                Random.value * 20f + 10f); // each time we call AddPointToCategory 
            Graph.DataSource.AddPointToCategory("Player 2", DateTime.Now + TimeSpan.FromDays(x),
                Random.value * 20f + 10f); // each time we call AddPointToCategory 
