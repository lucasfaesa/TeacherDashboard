...
namespace ChartAndGraph
{
    ...
    public partial class CanvasLines : EventHandlingGraphic
    {
        ...
        /// <summary>
        ///     Tiling value for the graphic. The tiliing is the total length of the lines being drawn. It is set by the parent
        ///     graph
        /// </summary>
        public float Tiling { get; set; } = 1f;

        private bool mNegativeFill = true;
        ...
        /// </summary>
        private float mPointSize = 5f;

        /// <summary>
        ///     for fill render mode , this sets the boundries of the fill. (so the bottom of the fill matches the bottom of the
        ///     graph)
        /// </summary>
        ...
        /// <summary>
        ///     list of lines for the line object , this will be used to render either dots , fill or lines depending on the render
        ///     mode
        /// </summary>
        private List<LineSegement> mLines;
        ...
        internal struct Line
        {
            public Line(Vector3 from, Vector3 to, float halfThickness, bool hasNext, bool hasPrev) : this()
            {
                ...
                From = from;
                To = to;
                Normal = new Vector3(Dir.y, -Dir.x,
                    Dir.z); // this part calculates the line inset and points based on thichkness
        ...
        {
            private readonly List<Vector4> mLines = new();
            ...
            /// <summary>
            ///     gets the line at the specified index.index Must be below LineCount. if the line has a previous or a next line an
            ///     inset is calculated for better visual aperance
            /// </summary>
            /// <param name="index"></param>
            /// <param name="from"></param>
            /// <param name="to"></param>
        ...
        private void FindBoundingValues()
        {
            ...
            if (mLines != null)
                ...
                {
                    var seg = mLines[i];
                    var totalPoints = seg.PointCount;
        ...
        /// <summary>
        ///     this method inflate one point in a line with the specified distantce. This enable the graphic to control the
        ///     thickness of lines
        /// </summary>
        ...
        /// <param name="p1"></param>
        /// <param name="p2"></param>
        private void GetSide(Vector3 point, Vector3 dir, Vector3 normal, float dist, float size, float z,
        ...
        /// <summary>
        ///     overriding the default implementation to support materials with _ChartTiling property. This is used to tile texture
        ///     along the graphic lines
        /// </summary>
        ...
        protected override Vector2 Min => new(mMinX, mMinY);

        protected override Vector2 Max => new(mMaxX, mMaxY);

        protected override float MouseInThreshold => Mathf.Max(Thickness, mPointSize) + Sensitivity;
        ...
        protected override void SetUpHoverObject(ChartItemEffect hover, int index, int type, object data)
        {
            ...
            if (mPointRender)
            {
                if (index >= mLines[0].PointCount)
                    return;
                var point = mLines[0].getPoint(index);
                var transform = hover.GetComponent<RectTransform>();
        ...
        private IEnumerable<UIVertex> getDotVeritces()
        {
            if (mLines == null)
                yield break;
            var z = 0f;
            var halfSize = mPointSize * 0.5f;
            for (var i = 0; i < mLines.Count; ++i)
            {
                var seg = mLines[i];
                var total = seg.PointCount;
                for (var j = mMinModifyIndex; j < total; ++j)
                {
                    var magPoint = seg.getPoint(j);
                    if (magPoint.w == 0f)
                        continue;

                    Vector3 point = magPoint;
                    halfSize = mPointSize * 0.5f;
                    ProcesssPoint(ref magPoint, ref halfSize);
                    var p1 = point + new Vector3(-halfSize, -halfSize, 0f);
                    var p2 = point + new Vector3(halfSize, -halfSize, 0f);
                    var p3 = point + new Vector3(-halfSize, halfSize, 0f);
                    var p4 = point + new Vector3(halfSize, halfSize, 0f);
                    var uv1 = new Vector2(0f, 0f);
                    var uv2 = new Vector2(1f, 0f);
                    var uv3 = new Vector2(0f, 1f);
                    var uv4 = new Vector2(1f, 1f);

                    var v1 = ChartCommon.CreateVertex(p1, uv1, z);
                    var v2 = ChartCommon.CreateVertex(p2, uv2, z);
                    var v3 = ChartCommon.CreateVertex(p3, uv3, z);
                    var v4 = ChartCommon.CreateVertex(p4, uv4, z);

                    if (ClipRect.HasValue == false || ClipRect.Value.Contains(p1) || ClipRect.Value.Contains(p2) ||
                        ClipRect.Value.Contains(p3)
        ...
        private IEnumerable<UIVertex> getFillVeritces()
        {
            if (mLines == null)
                yield break;
            var z = 0f;
            ...
            {
                var seg = mLines[i];
                var totalLines = seg.LineCount;
                for (var j = mMinModifyIndex; j < totalLines; ++j)
                {
                    Vector3 from;
                    Vector3 to;
                    ...
                    var fromU = (from.x - mFillRect.xMin) / mFillRect.width;
                    var toU = (to.x - mFillRect.xMin) / mFillRect.width;
                    var uv1 = TransformUv(new Vector2(fromU, fromV));
                    var uv2 = TransformUv(new Vector2(toU, toV));
                    var uv3 = TransformUv(new Vector2(fromU, 0f));
                    var uv4 = TransformUv(new Vector2(toU, 0f));

                    var v1 = ChartCommon.CreateVertex(from, uv1, z);
                    var v2 = ChartCommon.CreateVertex(to, uv2, z);
                    var v3 = ChartCommon.CreateVertex(fromBottom, uv3, z);
                    var v4 = ChartCommon.CreateVertex(toBottom, uv4, z);


                    if ((from.y < mFillZero) ^ (to.y < mFillZero))
                    {
                        var crossing = ChartCommon.LineCrossing(from, to, mFillZero);
                        var crossU = (crossing.x - mFillRect.xMin) / mFillRect.width;
                        var uvCross = TransformUv(new Vector2(crossU, 0f));
                        var vCross = ChartCommon.CreateVertex(crossing, uvCross, z);
        ...
        private IEnumerable<UIVertex> getLineVertices()
        {
            if (mLines == null)
                yield break;
            ...
            for (var i = 0; i < mLines.Count; ++i)
            {
                ...
                for (var j = mMinModifyIndex; j < totalLines; ++j)
                {
                    ...
                    var uv1 = new Vector2(tileUv * Tiling, 0f);
                    var uv2 = new Vector2(tileUv * Tiling, 1f);
                    tileUv += line.Mag / totalUv;

                    var uv3 = new Vector2(tileUv * Tiling, 0f);
                    var uv4 = new Vector2(tileUv * Tiling, 1f);
                    ...
                    if (peek.HasValue)
                    {
                        var myZ = z + 0.2f;
                        Vector3 a1, a2;
                        GetSide(line.To, line.Dir, line.Normal, halfThickness * 0.5f, halfThickness * 0.6f,
                            v3.position.z, out a1, out a2);
                        yield return v3;
                        yield return v4;
                    ...
                    if (prev.HasValue)
                    {
                        var myZ = z + 0.2f;
                        Vector3 a1, a2;
                        GetSide(line.From, -line.Dir, line.Normal, halfThickness * 0.5f, halfThickness * 0.6f,
                            v1.position.z, out a1, out a2);
        ...
        private Mesh mVHMesh;
        private readonly List<Vector3> mPositions = new();
        private readonly List<Vector2> mUvs = new();
        private readonly List<int> mTringles = new();

        private void WriteTo<T>(List<T> list, int index, T val)
        {
            if (list.Count == index)
                list.Add(val);
            else
...
#pragma warning disable 0672
...
#endif
        private void PickLine(Vector3 mouse, out int segment, out int line, out object selectionData)
        {
            ...
            {
                var seg = mLines[i];
                var total = seg.LineCount;
                for (var j = 0; j < total; ++j)
                {
                    Vector3 from;
                    Vector3 to;
                    seg.GetLine(j, out from, out to);
                    var dist = ChartCommon.SegmentPointSqrDistance(from, to, mouse);
                    if (dist < minDist)
                    {
                        minDist = dist;
                        segment = i;
            ...
            var thresh = Thickness + Sensitivity;
        ...
        private void PickDot(Vector3 mouse, out int segment, out int point, out object selectionData)
        {
            var minDist = Mathf.Infinity;
            segment = -1;
            point = -1;
            ...
            for (var i = 0; i < mLines.Count; ++i)
            {
                ...
                for (var j = 0; j < total; ++j)
                {
                    var p = seg.getPoint(j);
                    if (p.w == 0f)
                        continue;
                    var dist = (mouse - (Vector3)p).sqrMagnitude;
                    if (dist < minDist)
                    {
                        mag = p.w;
                        if (mag < 0f)
        ...
        private void TrimItem(float x1, float y1, float x2, float y2, bool xAxis, bool oposite, ref Vector2 from,
            ref Vector2 to)
        {
            var seg1 = new Vector2(x1, y1);
            var seg2 = new Vector2(x2, y2);
            Vector2 point;
        ...
        private void TrimLine(Rect r, ref Vector2 from, ref Vector2 to)
        {
