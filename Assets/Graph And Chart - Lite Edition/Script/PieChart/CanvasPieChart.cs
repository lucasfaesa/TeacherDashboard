...
namespace ChartAndGraph
{
    ...
    {
        [SerializeField] [Tooltip("prefab for the pie item. must contain a PieCanvasGenerator component")]
        private PieCanvasGenerator prefab;

        [SerializeField] [Tooltip("the thickness of the guideline between each slice and it's label")]
        private float lineThickness = 1f;

        [SerializeField] [Tooltip("The line spacing for each category label line")]
        private float lineSpacing = 20f;

        [SerializeField] [Tooltip("The line material for each category label line")]
        ...
        protected override float InnerDepthLink => 0f;

        protected override float OuterDepthLink => 0f;

        protected override Material LineMaterialLink => lineMaterial;

        protected override float LineThicknessLink => lineThickness;

        protected override float LineSpacingLink => lineSpacing;
        ...
        public override void InternalGenerateChart()
        {
            if (FitToContainer)
            {
                ...
                if (rect != null)
                {
                    var finalRadius = Math.Min(rect.rect.size.x, rect.rect.size.y) * 0.5f;
                    radius = finalRadius;
                }
            }
        ...
        protected override IPieGenerator PreparePieObject(out GameObject pieObject)
        {
            if (Prefab == null)
                ...
                pieObject = Instantiate(Prefab.gameObject);
